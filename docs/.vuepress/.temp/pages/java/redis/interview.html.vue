<template><h2 id="为什么说redis是单线程的" tabindex="-1"><a class="header-anchor" href="#为什么说redis是单线程的" aria-hidden="true">#</a> 为什么说REDIS是单线程的？?</h2>
...
<h2 id="缓存预热。" tabindex="-1"><a class="header-anchor" href="#缓存预热。" aria-hidden="true">#</a> 缓存预热。</h2>
...
<h2 id="缓存降级" tabindex="-1"><a class="header-anchor" href="#缓存降级" aria-hidden="true">#</a> 缓存降级</h2>
...
<h2 id="redis到底有多快" tabindex="-1"><a class="header-anchor" href="#redis到底有多快" aria-hidden="true">#</a> REDIS到底有多快？</h2>
...
<p><img src="@source/java/redis/assets/1574821356723.png" alt="1574821356723"></p>
<ol>
<li>完全基于内存</li>
...
<h3 id="redis-16-个常见使用场景" tabindex="-1"><a class="header-anchor" href="#redis-16-个常见使用场景" aria-hidden="true">#</a> Redis 16 个常见使用场景</h3>
<p>1、缓存
2、数据共享分布式
3、分布式锁
4、全局ID
...
15、用户关注、推荐模型
16、排行榜</p>
<h2 id="redis-的集群方式" tabindex="-1"><a class="header-anchor" href="#redis-的集群方式" aria-hidden="true">#</a> Redis 的集群方式</h2>
...
<p>主从集群分两种：</p>
<p><img src="@source/java/redis/assets/1574821993599.png" alt="1574821993599"> <img src="@source/java/redis/assets/1574822026037.png" alt="1574822026037"></p>
<p>带有哨兵的集群：</p>
<p><img src="@source/java/redis/assets/1574822077190.png" alt="1574822077190"></p>
<p>2）分片集群</p>
<p>主从集群中，每个节点都要保存所有信息，容易形成木桶效应。并且当数据量较大时，单个机器无法满足需求。此时我们就要使用分片集群了。</p>
<p><img src="@source/java/redis/assets/1574822184467.png" alt="1574822184467"></p>
<p>集群特征：</p>
<ul>
<li>
<p>每个节点都保存不同数据</p>
...
<p><img src="@source/java/redis/assets/1574822584357.png" alt="1574822584357"></p>
<p>出现故障时，主从可以及时切换：</p>
<p><img src="@source/java/redis/assets/1574822602109.png" alt="1574822602109"></p>
<h2 id="redis-的常用数据类型" tabindex="-1"><a class="header-anchor" href="#redis-的常用数据类型" aria-hidden="true">#</a> Redis 的常用数据类型</h2>
...
<h2 id="redis-事务机制" tabindex="-1"><a class="header-anchor" href="#redis-事务机制" aria-hidden="true">#</a> Redis 事务机制</h2>
...
<h3 id="redis-不支持回滚" tabindex="-1"><a class="header-anchor" href="#redis-不支持回滚" aria-hidden="true">#</a> Redis 不支持回滚</h3>
<p>优点：</p>
<ul>
...
<h2 id="事务执行一半的时候-redis-宕机怎么办" tabindex="-1"><a class="header-anchor" href="#事务执行一半的时候-redis-宕机怎么办" aria-hidden="true">#</a> 事务执行一半的时候 Redis 宕机怎么办？</h2>
...
<h2 id="redis-的-key-过期策略" tabindex="-1"><a class="header-anchor" href="#redis-的-key-过期策略" aria-hidden="true">#</a> Redis 的 Key 过期策略</h2>
<h4 id="为什么需要内存回收" tabindex="-1"><a class="header-anchor" href="#为什么需要内存回收" aria-hidden="true">#</a> 为什么需要内存回收？</h4>
<ul>
...
<h4 id="过期删除策略" tabindex="-1"><a class="header-anchor" href="#过期删除策略" aria-hidden="true">#</a> 过期删除策略</h4>
<p>删除达到过期时间的 key。</p>
<ul>
<li>1）定时删除</li>
</ul>
...
<h4 id="内存淘汰策略" tabindex="-1"><a class="header-anchor" href="#内存淘汰策略" aria-hidden="true">#</a> 内存淘汰策略</h4>
...
<h3 id="maxmemory-policy-可以配置要使用哪一个淘汰机制" tabindex="-1"><a class="header-anchor" href="#maxmemory-policy-可以配置要使用哪一个淘汰机制" aria-hidden="true">#</a> maxmemory-policy 可以配置要使用哪一个淘汰机制</h3>
<p>什么时候会进行淘汰？</p>
...
<h3 id="redis-过期策略" tabindex="-1"><a class="header-anchor" href="#redis-过期策略" aria-hidden="true">#</a> Redis 过期策略</h3>
...
<h2 id="redis-在项目使用" tabindex="-1"><a class="header-anchor" href="#redis-在项目使用" aria-hidden="true">#</a> Redis 在项目使用</h2>
<h3 id="共享-session" tabindex="-1"><a class="header-anchor" href="#共享-session" aria-hidden="true">#</a> 共享 session</h3>
...
<h3 id="异步队列" tabindex="-1"><a class="header-anchor" href="#异步队列" aria-hidden="true">#</a> 异步队列</h3>
<p>Reids 在内存存储引擎领域的一大优点是提供 list 和 set 操作，这使得 Redis 能作为一个很好的消息队列平台来使用。而且 Redis 中还有 pub/sub 这样的专用结构，用于 1 对 N 的消息通信模式。</p>
<h2 id="redis-的缓存击穿、缓存雪崩、缓存穿透" tabindex="-1"><a class="header-anchor" href="#redis-的缓存击穿、缓存雪崩、缓存穿透" aria-hidden="true">#</a> Redis 的缓存击穿、缓存雪崩、缓存穿透</h2>
...
<h2 id="数据库与缓存数据一致性" tabindex="-1"><a class="header-anchor" href="#数据库与缓存数据一致性" aria-hidden="true">#</a> 数据库与缓存数据一致性</h2>
<p>实现方案：</p>
<ul>
...
<h2 id="redis-存储对象信息是用-hash-还是-string" tabindex="-1"><a class="header-anchor" href="#redis-存储对象信息是用-hash-还是-string" aria-hidden="true">#</a> Redis 存储对象信息是用 Hash 还是 String</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br></div></div><p>如果此时将此用户信息的 name 改为“xiaoli”，再存到 Redis 中，Redis 是不需要重新分配空间的。而且我们在读取和存储数据的时候只需要对做 Json 序列化与反序列化，比较方便。</p>
<p>Hash
...
<div class="language-json ext-json line-numbers-mode"><pre v-pre class="language-json"><code><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br></div></div><p>那么该如何存储&quot;clothes&quot;属性又变成了该用 String 还是 Hash 的问题。</p>
