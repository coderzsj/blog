...
<h3 id="事务的4个特性acid" tabindex="-1"><a class="header-anchor" href="#事务的4个特性acid" aria-hidden="true">#</a> 事务的4个特性ACID</h3>
<p>原子性（Atomicity）：操作这些指令时，要么全部执行成功，要么全部不执行。只要其中一个指令执行失败，所有的指令都执行失败，数据进行回滚，回到执行指令前的数据状态。</p>
<p>一致性（Consistency）<Badge text="总量不变" />：事务的执行使数据从一个状态转换为另一个状态，数据库的完整性约束没有被破坏。</p>
<blockquote>
<p>拿转账来说，假设用户A和用户B 两者的钱加起来一共是2000，那么不管A和B之间如何转账，转几次账，事务结束后两个用户的钱相加起来应该还得是2000，这就是事务的一致性。</p>
</blockquote>
<p>隔离性（Isolation）<Badge text="信息彼此独立，互不干扰" />：隔离性是当多个用户并发访问数据库时，比如操作同一张表时，数据库为每一个用户开启的事务，不能被其他事务的操作所干扰，多个并发事务之间要相互隔离。</p>
<blockquote>
<p>即要达到这么一种效果：对于任意两个并发的事务T1和T2，在事务T1看来，T2要么在T1开始之前就已经结束，要么在T1结束之后才开始，这样每个事务都感觉不到有其他事务在并发地执行。</p>
</blockquote>
<p>持久性（Durability）：当事务正确完成后，它对于数据的改变是永久性的。不会轻易丢失</p>
<div class="language-sql ext-sql line-numbers-mode"><pre v-pre class="language-sql"><code><span class="token keyword">begin</span> <span class="token keyword">transaction</span>； 
<span class="token comment">// 本地数据库操作：张三减少金额 </span>
<span class="token comment">// 本地数据库操作：李四增加金额 </span>
<span class="token keyword">commit</span> transation<span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br></div></div><p>随着互联网的快速发展，软件系统由原来的单体应用转变为分布式应用，下图描述了单体应用向微服务的演变：分布式系统会把一个应用系统拆分为可独立部署的多个服务，因此需要服务与服务之间远程协作才能完成事务操 作，这种分布式系统环境下由不同的服务之间通过网络远程协作完成事务称之为分布式事务，例如用户注册送积分 事务、创建订单减库存事务，银行转账事务等都是分布式事务。</p>
...
<span class="token keyword">commit</span> transation<span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br></div></div><h3 id="cap定理" tabindex="-1"><a class="header-anchor" href="#cap定理" aria-hidden="true">#</a> CAP定理</h3>
<p>CAP原则指的是在一个分布式系统中，不可能同时满足以下三点。</p>
<p>一致性（Consistency）<Badge text="副本最新" /></p>
<p>指强一致性，在写操作完成后开始的任何读操作都必须返回该值，或者后续写操作的结果。</p>
<p>可用性（Availability）<Badge text="高可用" />：</p>
<p>可用性是指，每次向未崩溃的节点发送请求，总能保证收到响应数据（允许不是最新数据）</p>
<p>分区容忍性（Partition tolerance）<Badge text="能容忍网络分区" />：分布式系统在遇到任何网络分区故障的时候，仍然能够对外提供满足一致性和可用性的服务,，也就是说，服务器A和B发送给对方的任何消息都是可以放弃的，也就是说A和B可能因为各种意外情况，导致无法成功进行同步，分布式系统要能容忍这种情况。除非整个网络环境都发生了故障。</p>
<ol>
<li>使用MQ</li>
<li>使用Seata</li>
</ol>
...
<h3 id="一致性" tabindex="-1"><a class="header-anchor" href="#一致性" aria-hidden="true">#</a> 一致性 <Badge text="数据一致性" /></h3>
...
<h2 id="at事务模式" tabindex="-1"><a class="header-anchor" href="#at事务模式" aria-hidden="true">#</a> AT事务模式</h2>
<p>分布式事务工作机制</p>
...
<h2 id="sagas事务模式" tabindex="-1"><a class="header-anchor" href="#sagas事务模式" aria-hidden="true">#</a> sagas事务模式</h2>
<p>最终一致性</p>
...
<p><img src="@source/java/soa/assets/20220413/分布式事务-1649804696250.png" alt="saga案例"></p>
...
<h2 id="tcc事务模型" tabindex="-1"><a class="header-anchor" href="#tcc事务模型" aria-hidden="true">#</a> TCC事务模型</h2>
<p>TCC(Try Confirm Cancel)事务模型的思想和2PC提交有点类似。下图是TCC和2PC(XA)的对比。</p>
<p><img src="@source/java/soa/assets/20220413/分布式事务-1649805216922.png" alt="TCC实物模型的思想和2PC提交比较"></p>
<ol>
...
<p>Confirm（确认）：执行真正的业务执行业务 不做任何业务检查</p>
<ul>
<li>只使用Try阶段预留的业务资源</li>
<li>释放Try阶段预留的业务资源</li>
</ul>
<p>Cancle（取消） - 释放Try阶段预留的业务资源 以下单扣库存为例，</p>
<ul>
<li>Try 阶段去占库存（资源的检测与预留）；</li>
<li>Confirm 阶段则实际扣库存（执行的业务操作提交）；</li>
<li>如果库存扣减失败 Cancel 阶段进行回滚，释放库存（预留资源释放）；</li>
</ul>
<h3 id="一个完整的tcc事务参与方包括三部分" tabindex="-1"><a class="header-anchor" href="#一个完整的tcc事务参与方包括三部分" aria-hidden="true">#</a> 一个完整的TCC事务参与方包括三部分：</h3>
<ul>
<li>发起方</li>
...
<h3 id="tcc-案例" tabindex="-1"><a class="header-anchor" href="#tcc-案例" aria-hidden="true">#</a> tcc 案例</h3>
...
<p><a href="https://gitee.com/SHIJINGSPACE/seata-tcc-main" target="_blank" rel="noopener noreferrer">实例代码<ExternalLinkIcon/></a></p>
<h3 id="一个稳定的、高可用的、扩展性强的tcc事务管理器-seata" tabindex="-1"><a class="header-anchor" href="#一个稳定的、高可用的、扩展性强的tcc事务管理器-seata" aria-hidden="true">#</a> 一个稳定的、高可用的、扩展性强的TCC事务管理器（seata）</h3>
<h3 id="tcc事务的优点和限制" tabindex="-1"><a class="header-anchor" href="#tcc事务的优点和限制" aria-hidden="true">#</a> TCC事务的优点和限制</h3>
...
<h2 id="基于消息的分布式事务" tabindex="-1"><a class="header-anchor" href="#基于消息的分布式事务" aria-hidden="true">#</a> 基于消息的分布式事务</h2>
